...
export const updater = (store: RecordSourceSelectorProxy) => {
  console.log("hello");
  const commentNode = store.getRootField('CommentAdded').getLinkedRecord('comment');
  ...
  if (!commentStore) {
    const commentConnection = ConnectionHandler.getConnection(store.getRoot(), 'CommentList_comments');

    // create user edge
    if(commentConnection){
      const commentEdge = ConnectionHandler.createEdge(store, commentConnection, commentNode, 'CommentEdge');
